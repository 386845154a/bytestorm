...
public class RoleServiceImpl implements IRoleService
{
    ...
    public int deleteRoleByIds(String ids) throws BusinessException
    {
        ...
        for (Long roleId : roleIds)
        {
            ...
            if (countUserRoleByRoleIdExcludeCurrentUser(roleId) > 0)
            {
                throw new BusinessException(String.format("%1$s已分配,不能删除", role.getRoleName()));
            }
        }
        //删除角色的时候清除角色与用户关联表
        userRoleMapper.deleteUserRoleByRoleId(roleIds);
    ...
    public int insertRole(Role role)
    {
        ...
        //新增角色时，哪个用户添加的就和哪个用户做关联(系统管理员除外)
        User currentUser = ShiroUtils.getSysUser();
        if(!checkUserAllowed(currentUser)){
            insertUserRole(currentUser.getUserId(),new Long[]{role.getRoleId()});
        }
     ...
     * @param user
     */
    public Boolean checkUserAllowed(User user)
    {
    ...
    /**
     * 通过角色ID查询角色使用数量(排除当前登录用户)
     *
     * @param roleId 角色ID
     * @return 结果
     */
    public int countUserRoleByRoleIdExcludeCurrentUser(Long roleId)
    {
        Map<String,Object> map=new HashMap<String,Object>();
        map.put("roleId",roleId);
        map.put("currentUserId",ShiroUtils.getUserId());
        return userRoleMapper.countUserRoleByRoleIdExcludeCurrentUser(map);
    }
